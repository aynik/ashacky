...
## Optional FIDO2 trial (2026-09-10)

The candidate adds the host CTAP/Secure Enclave service and a standard-library Python UHID relay over the existing private control port. The reference app and guest payload were activated after an authorized clean restart, with prior-file backups. Generated host plans still default to `fido2Enabled: false`, and staging lists the guest unit separately from normally enabled services. The trial does not change PAM or GNOME Keyring. The optional guest service was initially left disabled for boot and was enabled after the owner accepted the tested scope below.

All 103 source tests pass. The additional checks cover HID fragmentation, channel isolation/resynchronization, cancellation and stale replies, request deadlines, private broker access and cancellation while authentication waits. Yubico's low-level CTAP client verifies ES256 registration self-attestation, discoverable/allow-list assertions, RP binding and malformed-message rejection using a Swift software signer that exists only in the fixture. Known-ID pre-flight responses perform no key operation, contain no user information, leave UP/UV unset and fail signature verification; normal authenticated responses verify correctly. Strict-protection and wrong-RP probes are rejected.

A disposable real UHID fixture passes INIT/GetInfo and a 1024-byte fragmented PING, and an extra temporary udev rule validates its parent-property identity match and active-seat access tag. The fixture is removed after the check. Native host tests cover private credential-file persistence, permissions, symlink rejection and full-store resident replacement using unusable key data; they never prompt. Fresh-plan generation and isolated Debian payload staging pass without installation. The localhost WebAuthn fixture passed registration and both assertion paths with test-only software keys, and its JavaScript parses in SpiderMonkey.

The assembled candidate passes relocation, all 52 Mach-O dependency closures/signatures, existing control/session/launcher checks, Metal shaders and the silent audio pipeline. After activation, GetInfo succeeded through the private channel and the ordinary desktop user could discover/open the virtual hidraw device.

The first browser trial was interrupted by a reported frozen desktop. Guest journals show SPICE status/control updates timing out shortly after Firefox started; macOS still processed frontend window/input events, and the VM subsequently shut down cleanly during recovery. No native authentication evaluation or saved Secure Enclave credential was found. There was no thread sample from the stalled process, so the blocking call and cause remain unconfirmed. The optional guest service remained inactive after recovery. Do not count this attempt as browser or native-authentication acceptance.

Recovery exposed a separate startup defect: Foundation's non-recursive directory creation rejected the already-existing private credential directory. The fix uses `mkdir` followed by the existing ownership/type/mode validation, accepting an existing safe directory without changing its permissions. Native fixtures now reopen both empty and populated stores and reject directory symlinks and permissive modes. A separately assembled candidate passed those checks and the runtime suite and was activated with the SPICE repair in a subsequent authorized clean restart. Host command/status and localhost test-phase logs now omit all RP/user/credential data while making the request stage observable.

After that restart, the installed app hashes matched the tested repair and strict signature verification passed. The existing device services and restricted power check passed. The optional guest service was then started explicitly without enabling it for boot, and GetInfo succeeded as the ordinary desktop user. A detached disposable Firefox profile opened the running localhost fixture. The owner confirmed it works; the server independently verified registration, allow-list sign-in and discoverable sign-in, including fresh challenge, origin/RP, assertion signature and UP/UV flags. Corresponding native CTAP operations completed successfully, one protected credential record was saved with mode 0600, and control/transport services remained active with no automatic restarts. No real website account was enrolled.

The owner subsequently confirmed native Mac account-password fallback succeeds and accepted the tested scope. The reference guest service is now enabled for boot, with the prior enablement state and deferred checks recorded privately. The disposable browser was already closed; the localhost test server was stopped. The public test credential and protected host record are retained for a future persistence check, and recovery backups remain. GNOME Keyring still requests its own unlock password, as expected for this separate integration.

Native cancellation, real-credential persistence after restart and screen-lock behavior remain untested. Python-fido2 2.2.1's high-level client has a PIN-negotiation limitation documented in [FIDO2.md](FIDO2.md); success with the tested Firefox client is not a claim of universal client compatibility.

The final source check passes all 103 tests and checks 182 source files for syntax and private-environment patterns. The guide audit validates 32 shell blocks and 56 relative links/anchors. Test profiles, credential state, deployment receipts, recovery apps and generated builds remain outside the source payload.

## SPICE display acknowledgement under backpressure (2026-09-10)

The owner clarified that the desktop froze as Firefox opened, before any passkey button could be clicked. Subsequent bounded checks opened and closed a disposable blank Firefox profile, started/stopped a silent guest audio stream, and displayed the test page with a disposable virtual HID that rejected all authentication. SPICE telemetry continued updating and host thread samples showed its loop responding. The HID fixture received no CTAP requests. These checks did not reproduce the earlier stall or test native authentication.

Code review found that asynchronous Metal completion called a SPICE display-acknowledgement API whose implementation used `spice_msg_out_send_internal`, a coroutine-only direct socket write. An isolated native socket-pair fixture reproduced the defect: an unblocked call wrote directly instead of queuing; a full socket with an already-waiting display coroutine reached an invalid `g_coroutine_socket_wait` assertion. The local patch uses the existing `spice_msg_out_send` queue so the channel coroutine performs the write. It does not move frame acknowledgement before the Metal copy completes.

The same fixture linked against the assembled candidate returns promptly and queues the acknowledgement with both free and full sockets. All 11 upstream SPICE/common tests, 103 Ashacky source tests and the full runtime checks pass, including 52 Mach-O dependency closures, signatures, Metal shaders and the private FIDO storage reopening checks. The candidate was built with an isolated SPICE dependency prefix and activated through an authorized clean restart. Bundle hashes and signatures match the tested build. The attended Firefox registration and sign-in operations completed, with advancing guest telemetry and no automatic control/transport restarts. The old process was not sampled during its stall, so this demonstrates and fixes a transport defect consistent with the symptoms, not a confirmed reconstruction of that incident. Broader desktop workloads and sleep/wake remain separate checks.

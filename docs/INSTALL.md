...
- The Bluetooth D-Bus facade owns `org.bluez`. Record and disable/mask stock bluetooth.service only for this dedicated guest; don't run two owners concurrently. Keep its D-Bus policies/packages available. Install rfkill support and the facade's `bluetooth_events.py` companion from the layout. Verify its event mode, radio heartbeat and device changes using [the Bluetooth acceptance checks](DEVELOPMENT.md#updating-bluetooth-event-delivery).
- Audio nodes expose host default-device selection over SPICE, not independent simultaneous physical routes. Install the audio script and its companion module from the layout, and enable the user PipeWire integration after the audio stack. Verify `Audio host updates: events` and device hotplug using [the audio acceptance checks](DEVELOPMENT.md#updating-audio-event-delivery).

...
The optional FIDO2 prototype is included in the frontend build using system frameworks. Its guest relay uses Python's standard library and is staged with a separately listed, disabled service. Independent Yubico-client, disposable UHID and attended authentication checks are described in [FIDO2.md](FIDO2.md#build-and-checks); `python3-fido2` is a test dependency, not a runtime requirement.
...
`check-runtime.py` copies the generated app to a temporary path containing spaces, verifies signatures and all Mach-O dependencies, queries QEMU's devices/HVF/GPU properties, loads the graphics and codec libraries, tests GStreamer with a silent `fakesink`, and creates the Metal shader pipeline. It opens no desktop window, VM, audio device, microphone, camera, network listener or privileged service. It also tests the bundled launcher against a temporary, windowless app with a unique test identity, covering exits, startup cancellation and forced termination. This requires an active macOS GUI account; the fixture requests no permissions and its LaunchServices registration and files are removed afterward. This validates relocation and component availability; it does not validate a fresh guest boot.

It also runs `tools/check-spice-client.py` against the app's actual bundled SPICE library. A disposable socket-pair fixture checks that display acknowledgements enqueue and return even when the socket is full. The checker uses private headers and configuration macros from the matching `build/runtime/spice-gtk` compilation database; pass `--spice-build PATH` to `check-runtime.py` when validating an isolated dependency build elsewhere. `patches/spice-gtk-display-ack.patch` is part of the normal dependency recipe. Rebuilding only the frontend against an older SPICE client does not include this fix and fails the new check.

...
`org.ashacky.control` is a bidirectional SPICE port attached to the existing virtio-serial controller. It carries Wi-Fi, Bluetooth/audio, power, Touch ID, camera lifecycle/frame descriptors and explicit guest-to-host lock requests. Camera pixels, audio samples, decoded video and public status snapshots do not pass through it. There is no network address discovery or SSH connection for control, so changing the host Wi-Fi connection cannot interrupt this path.

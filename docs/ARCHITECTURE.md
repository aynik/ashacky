...
flowchart TB
  ...
  Frontend -->|camera pixels in separate shared memory| Guest
...
Metal completes a scanout copy asynchronously, then acknowledges the frame on the SPICE main context. The local SPICE client patch queues that acknowledgement for its channel coroutine instead of writing directly to the socket. A full socket must not make the shared event loop attempt a coroutine wait: that loop also delivers input, audio and the private control/status ports. Frame ownership is still released only after the copy completes.
...
| Sidecar display controls | Existing GNOME extension and virtual GPU outputs | On-demand discovery/connect/disconnect over private SPICE control; an embedded child uses macOS Sidecar. The three-output candidate for built-in, wired and iPad screens passed automated checks; simultaneous hardware testing and activation are deferred. See [SIDECAR.md](SIDECAR.md). |
...
The guest root agent authenticates local peer credentials and the configured desktop UID. Device and session control use the private channel described below. Camera demand and frame descriptors use that same authenticated channel; pixels use separate shared memory. No runtime integration uses SSH. The optional administrative SSH tools remain separate from application transport.
...
`org.ashacky.control` is a bidirectional SPICE port attached to the existing virtio-serial controller. It carries Wi-Fi, Bluetooth/audio, power, Touch ID, optional FIDO2 requests, camera lifecycle/frame descriptors and explicit guest-to-host lock requests. Camera pixels, audio samples, decoded video and public status snapshots do not pass through it. There is no network address discovery or SSH connection for control, so changing the host Wi-Fi connection cannot interrupt this path.

`ashacky-control.service` owns the root-only guest port and mode-0600 Unix sockets `host.sock`, `wifi.sock`, `bluetooth.sock`, `camera.sock` and `fido2.sock` inside `/run/ashacky-control` (0700). Existing root helpers use their existing JSON operations there; desktop users still go through the peer-UID-checked agent and device facade. The optional FIDO2 transport instead exposes an active-seat hidraw device and forwards CTAP messages through its root-only socket. The udev rule keeps the private port root-owned at 0600. The app dispatches device requests to its existing private listeners; optional FIDO2 has its own asynchronous queue so CANCEL can invalidate a pending native prompt. Active-console checks, field validation, LocalAuthentication and the single permission identity remain. No guest request may reach the supervisor-only `vm-stopped` operation.

Protocol version 1 uses newline-delimited JSON, a fresh guest nonce and host session UUID, and the existing per-installation token on the private port handshake. Host-control RPC also retains its existing token check. Every later message is bound to that connection's session UUID; request IDs are increasing and replies are correlated independently. Frames are limited to 32 KiB, each output queue to 256 KiB, local guest clients to 32 and host service workers to 16, including workers surviving a disconnect. SPICE writes retain their data until completion, run one at a time and can be cancelled. A stalled write or handshake expires after ten seconds. Local requests expire after 20 seconds for devices or 80 seconds for host/FIDO2 authentication; request parsing and response delivery each have a five-second limit. Idle control traffic needs no heartbeat or polling loop.
...
## Camera shared memory and demand

The camera remains a standard V4L2 device backed by the existing v4l2loopback reader-demand events and FFmpeg NV12-to-YUYV conversion. No application-specific camera plugin is required. Its root service opens capture only when the usage monitor reports a reader; generated black frames keep the device discoverable and replace the last image after capture stops. Usage-monitor loss, writer exit and termination wake the existing event loop. The verified usage-event contract is v4l2loopback 0.15.4.

A separate 8 MiB `linuxhost-shmem` PCI BAR carries camera pixels, while decoded video retains its existing 64 MiB BAR. The same local PCI ID identifies both; their exact sizes distinguish their roles. The camera reader rejects missing or ambiguous matches and keeps both camera resource files root-only. The video setup skips the camera BAR and grants the desktop access only to the video BAR. QEMU needs no new patch or binary for the second mapping.

The supervisor creates a private mapping for each VM session. Ashacky writes 1280×720 video-range NV12 frames directly into three bounded 2 MiB slots, without building or transmitting a raw-frame message. `camera-start`, `camera-next` and `camera-stop` use the root-only `camera.sock` adapter on `org.ashacky.control`. Only small descriptors cross SPICE. One frame is leased to the reader until its next acknowledgement; a slow reader receives the newest queued frame and intermediate frames are dropped. A condition variable wakes a waiting request when a frame arrives, with a one-second stall deadline, instead of polling shared memory.

The common C memory helper publishes with a release commit and validates the UUID/sequence before and after the guest copy, using explicit memory barriers. The guest uses a read-only mapping and stable scratch storage before feeding FFmpeg. Revocation or a mismatched descriptor discards the frame. This avoids SSH encryption and stream copies, but retains the capture-to-shared-memory copy, the stable guest copy and the V4L2 conversion; it is not end-to-end zero-copy capture.

Capture remains in Ashacky.app under its existing Camera permission and active-console checks. Explicit stop, host sleep/session resignation or five seconds without renewed frame demand stops capture and clears the shared slots. The lease is a one-shot deadline renewed by successful requests, with no idle heartbeat or forced 30-second stream restart. The guest requests stop when demand ends and clears its scratch storage; a lost stop reply is covered by the host lease. Startup RPC allows six seconds, ordinary camera RPC two seconds, and failed active streams retry after half a second. Existing child termination deadlines remain. The mapping is a private, temporary memory-backing file; no media recording is created.

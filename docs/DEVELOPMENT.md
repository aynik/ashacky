...
The source consolidation retains compatibility names such as `linuxhost`, `probe` and `Workbench` in interfaces and directories. They do not imply that discarded prototypes should run. Retire a compatibility name only with an explicit migration for every consumer.

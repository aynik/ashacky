...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_usb_topology_preserves_four_explicit_root_ports(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn('linuxhost-shmem,memdev=camera-frames,addr=0xc', args)
            self.assertIn('linuxhost-shmem,memdev=video-frames,addr=0xb', args)
            self.assertTrue(any('id=camera-frames,size=8M' in value for value in args))
